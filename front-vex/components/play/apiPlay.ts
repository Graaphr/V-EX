...
export async function getHallModel(exhibitionId: string) {
    const res = await api.get(`/api/experience/3d-models/${exhibitionId}`)
    return res.data.model_hall
}

export async function getKaryaList(exhibitionId: string) {
    const res = await api.get(`/api/experience/karya/pameran/${exhibitionId}`)
...
export async function getPameranFolder(exhibitionId: string) {
    const res = await api.get(`/api/pameran/${exhibitionId}`)
...
export async function getGameAssets() {
    const res = await api.get("/api/experience/game-assets")
    return res.data
}

// ── page.tsx (ExhibitionPage) ──
export async function getPlayerName() {
    const res = await api.get("/api/player-name")
    return res.data.name as string
}

export async function deletePlayer(playerId: string) {
    await api.delete(`/api/player?id=${playerId}`)
}
...
export async function getKaryaLikeStatus(karyaId: number, token?: string) {
    const res = await api.get(`/api/karya/${karyaId}/suka`, {
...
export async function toggleKaryaLike(karyaId: number, token: string) {
    const res = await api.post(`/api/karya/${karyaId}/suka`, null, {
