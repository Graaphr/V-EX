...
export default function Booth({
  ...
  useEffect(() => {
    const check = async () => {
      const posterOk = await fileExists(poster);
  ...
  const loadTextureSafe = async (path: string, onLoad: (texture: THREE.Texture) => void) => {
  ...
  useEffect(() => {
    ...
    loadTextureSafe(poster, (texture) => {
  ...
  useEffect(() => {
    const setup = async () => {
      if (!canRender || !video || !videoMesh.current) return;

      const exists = await fileExists(video);
      ...
      videoElement.current = htmlVideo;

      const texture = new THREE.VideoTexture(htmlVideo);

      texture.colorSpace = THREE.SRGBColorSpace;

      videoMesh.current!.material = new THREE.MeshBasicMaterial({
        map: texture,
        toneMapped: false,
      });
    };
  ...
  const handleClick = (e: any) => {
    ...
    if (clicked === 'PanelVideo' && videoElement.current) {
  ...
  return (
    <group position={position} quaternion={new THREE.Quaternion(...quaternion)}>
